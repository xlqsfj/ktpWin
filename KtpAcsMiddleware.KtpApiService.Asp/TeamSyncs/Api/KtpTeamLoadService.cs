...
namespace KtpAcsMiddleware.KtpApiService.Asp.TeamSyncs.Api
{
    ...
    ///     使用开天平ASP接口进行班组拉取
    /// </summary>
    internal class KtpTeamLoadService
    {
        ...
        ///     拉取更新班组
        /// </summary>
        public void PullTeam(Team localTeam, int ktpTeamId)
        {
            var ktpTeam = GetKtpTeam(ktpTeamId);
            if (ktpTeam == null)
            {
        ...
        ///     获取开太平班组信息
        /// </summary>
        private KtpTeamGetApiResultContentPo GetKtpTeam(int ktpTeamId)
        {
            var ktpTeams = GetKtpTeams();
            if (ktpTeams == null || ktpTeams.Count == 0)
            {
        ...
        private IList<KtpTeamGetApiResultContentPo> GetKtpTeams()
        {
            ...
            try
            {
                apiResult = HttpClientHelper.Post(GetProjectTeamsApi, $"pro_id={ConfigHelper.ProjectId}");
