...
{
    public partial class WorkerDeviceInfo : Skin_Color
    {
        ...
        public WorkerDeviceInfo(dynamic dName)
        {
            InitializeComponent();
            LoadTeam();
            //加载控件
            ...
            _dName = dName;
            this.Text = "【" + dName + "】设备人员管理";
            //工人列表
            BindTeamWorkers(null);
        ...
        ///     设备列表绑定
        /// </summary>
        private void BindDevices(string _currentDeviceCode = null)
        {
            try
            {
                com_DeviceList.Items.Clear();
                com_DeviceList.Items.Add("所有");
                var _devices = ServiceFactory.FaceDeviceService.GetAll();
        ...
        private void TeamsLb_SelectedIndexChanged_1(object sender, EventArgs e)
        {
            if (TeamsLb.SelectedItem == null)
            {
                return;
        ...
        private void com_DeviceList_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            _dName = com_DeviceList.Text == "所有" ? null : com_DeviceList.Text;
            this.Text = "【" + _dName + "】设备人员管理";
            ktpGridPager.PageIndex = 1;
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void con_update_add_Click(object sender, EventArgs e)
        {
            if (WorkersGrid.CurrentRow != null)
            {
                try
                {
                    var panelStateCode = WorkersGrid.SelectedRows[0].Cells["panelStateCode"].Value;
                    var faceWorkerId = WorkersGrid.SelectedRows[0].Cells["dId"].Value.ToString();
                    var WorkerName = WorkersGrid.SelectedRows[0].Cells["WorkerName"].Value;
                    var state = (FaceWorkerState)panelStateCode;
                    //当前状态验证
                    ...
                    //确认提示;执行设置
                    if (MessageBox.Show($@"确认要将<{WorkerName}>设为新添加吗？", @"复位提示",
                            MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
                    {
