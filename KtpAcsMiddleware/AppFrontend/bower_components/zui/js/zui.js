...
+ function($) {
    ...
    $(document).on("click.zui.button.data-api",
        "[data-toggle^=button]",
        function(e) {
            var $btn = $(e.target);
...
(function($) {
    ...
    var Pager = function(element, options) {
        ...
        that.lang = $.isPlainObject(lang)
            ? ($.extend(true, {}, LANG[lang.lang || $.zui.clientLang()], lang))
            : LANG[lang];
    ...
    Pager.prototype.set = function(page, recTotal, recPerPage) {
        ...
        that.state = state;
        if (oldState.page !== state.page ||
            oldState.recTotal !== state.recTotal ||
            oldState.recPerPage !== state.recPerPage) {
    ...
    Pager.prototype.createLinkItem = function(page, text, asAElement) {
        ...
        var $ele = $('<a class="pager-item" data-page="' + page + '"/>')
            .attr("href", page ? that.createLink(page, that.state) : "###").html(text);
        if (!asAElement) {
            $ele = $("<li />").append($ele).toggleClass("active", page === that.state.page)
                .toggleClass("disabled", !page);
        }
        return $ele;
    };
    ...
    Pager.prototype.createGoto = function() {
        ...
        var $goto = $('<div class="input-group pager-goto" style="width: ' +
            (35 + (pager.page + "").length * 9 + 25 + that.lang.goto.length * 12) +
            'px"><input value="' +
            pager.page +
    ...
    Pager.prototype.createSizeMenu = function() {
        ...
        for (var i = 0; i < options.length; ++i) {
            ...
            var $li = $('<li><a href="###" data-size="' + size + '">' + size + "</a></li>")
                .toggleClass("active", size === pager.recPerPage);
            $menu.append($li);
        }
        return $(
            '<div class="btn-group pager-size-menu"><button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">' +
            that.lang.pageSize.format(pager) +
            ' <span class="caret"></span></button></div>').addClass(that.options.menuDirection).append($menu);
    };
    ...
    Pager.DEFAULTS = $.extend({
            elements: [
                "first_icon", "prev_icon", "pages", "next_icon", "last_icon", "page_of_total_text", "items_range_text",
                "total_text"
            ],
...
+ function($) {
    ...
    $(document).on("click.zui.tab.data-api",
...
+ function($) {
    ...
    $(document).on("click." + zuiname + ".data-api",
...
(function($) {
    ...
    var browseHappyTip = {
        'zh_cn':
            '您的浏览器版本过低，无法体验所有功能，建议升级或者更换浏览器。 <a href="http://browsehappy.com/" target="_blank" class="alert-link">了解更多...</a>',
        'zh_tw':
            '您的瀏覽器版本過低，無法體驗所有功能，建議升級或者更换瀏覽器。<a href="http://browsehappy.com/" target="_blank" class="alert-link">了解更多...</a>',
        'en':
            'Your browser is too old, it has been unable to experience the colorful internet. We strongly recommend that you upgrade a better one. <a href="http://browsehappy.com/" target="_blank" class="alert-link">Learn more...</a>'
    };
    ...
    Browser.prototype.tip = function(showCoontent) {
        ...
        $browseHappy.find(".content").html(showCoontent ||
            this.browseHappyTip ||
            browseHappyTip[$.zui.clientLang() || "zh_cn"]);
    };
                ...
                if (arguments.length <= 2 && typeof(args) == "object") {
                    for (var key in args) {
                        if (args[key] !== undefined) {
                            reg = new RegExp("(" +
                                (arguments[1] ? arguments[1].replace("0", key) : "{" + key + "}") +
                                ")",
                                "g");
...
(function($, document, Math) {
    ...
    Droppable.prototype.init = function() {
        ...
        var mouseMove = function(event) {
            ...
            // ignore small move
            if (Math.abs(mouseOffset.left - startMouseOffset.left) < deviation &&
                Math.abs(mouseOffset.top - startMouseOffset.top) < deviation) return;
            ...
            $targets.each(function() {
                ...
                if (mouseOffset.left > tX &&
                    mouseOffset.top > tY &&
                    mouseOffset.left < (tX + tW) &&
                    mouseOffset.top < (tY + tH)) {
...
+ function($) {
    ...
    Modal.prototype.escape = function() {
        if (this.isShown && this.options.keyboard) {
            $(document).on("keydown.dismiss." + zuiname,
                $.proxy(function(e) {
                        if (e.which == 27) {
    ...
    Modal.prototype.backdrop = function(callback) {
        ...
        if (this.isShown && this.options.backdrop) {
            ...
            this.$backdrop = $('<div class="modal-backdrop ' + animate + '" />')
                .appendTo(this.$body);
            this.$element.on("mousedown.dismiss." + zuiname,
                $.proxy(function(e) {
    ...
    $(document).on("click." + zuiname + ".data-api",
        ...
        function(e) {
            ...
            var option = $target.data(zuiname)
                ? "toggle"
                : $.extend({
            ...
            $target.one("show." + zuiname,
                function(showEvent) {
                    // only register focus restorer if modal will actually get shown
                    if (showEvent.isDefaultPrevented()) return;
                    $target.one("hidden." + zuiname,
                        function() {
...
(function($, window) {
    ...
    ModalTrigger.prototype.init = function(options) {
        ...
        $modal = $('<div id="' +
                ...
                (options.className || "") +
                '">' +
                (typeof options.loadingIcon === "string" && options.loadingIcon.indexOf("icon-") === 0
                    ? ('<div class="icon icon-spin loader ' + options.loadingIcon + '"></div>')
                    : options.loadingIcon) +
    ...
    ModalTrigger.prototype.show = function(option) {
        var options = $.extend({},
            ...
            {
                url: this.$trigger
                    ? (this.$trigger.attr("href") || this.$trigger.attr("data-url") || this.$trigger.data("url"))
                    : this.options.url
            },
            option);
        this.init(options);
    ...
    ModalTrigger.prototype.close = function(callback, redirect) {
        if (callback || redirect) {
            this.$modal.on("hidden" + ZUI_MODAL,
                function() {
    ...
    $.fn.modalTrigger = function(option, settings) {
        return $(this).each(function() {
            ...
            $this.on((options.trigger || "click") + ".toggle." + NAME,
                function(e) {
                    data.toggle(options);
                    if ($this.is("a")) e.preventDefault();
                });
    ...
    $(document).on("click." + NAME + ".data-api",
        '[data-toggle="modal"]',
        function(e) {
            var $this = $(this);
            var href = $this.attr("href");
...
+ function($) {
    ...
    Tooltip.prototype.enter = function(obj) {
        var self = obj instanceof this.constructor
            ? obj
            : $(obj.currentTarget)[this.type](this.getDelegateOptions()).data("zui." + this.type);
...
(function($, undefined) {
    ...
    var listenMouseMove = function() {
        $(document).off("mousemove." + NAME).on("mousemove." + NAME,
            function(e) {
                mouseX = e.clientX;
                mouseY = e.clientY;
            });
    ...
    var showContextMenu = function(items, options, callback) {
        ...
        var $target = $("#" + targetId);
        if (!$target.length) {
            $target = $('<div style="display: none; position: fixed; z-index: 2000;" class="contextmenu" id="' +
                targetId +
                '"><ul class="dropdown-menu contextmenu-menu"></ul></div>').appendTo("body");
        }
        var $menu = $target.find(".contextmenu-menu").off("click." + NAME).on("click." + NAME,
            "a",
            function(e) {
                var $item = $(this);
...
+ function($) {
    ...
    $(document).on("click.zui.carousel.data-api",
...
(function($, window, Math) {
    ...
    Lightbox.prototype.initGroups = function() {
        ...
        if (!groups) {
            groups = $('[data-toggle="lightbox"][data-group="' +
                this.options.group +
                '"], [data-lightbox-group="' +
                this.options.group +
                '"]');
    ...
    Lightbox.prototype.bindEvents = function() {
        ...
        $e.modalTrigger({
            ...
            custom: function(e) {
                ...
                modal.find(".prev").toggleClass("show",
                    groups.filter('[data-group-index="' + (groupIndex - 1) + '"]').length > 0);
                modal.find(".next").toggleClass("show",
                    groups.filter('[data-group-index="' + (groupIndex + 1) + '"]').length > 0);
                ...
                modal.find(".controller").click(function() {
                    ...
                    modal.find(".prev").toggleClass("show",
                        groups.filter('[data-group-index="' + (id - 1) + '"]').length > 0);
                    modal.find(".next").toggleClass("show",
                        groups.filter('[data-group-index="' + (id + 1) + '"]').length > 0);
                    return false;
                });
...
(function($, window, undefined) {
    ...
    var Messager = function(message, options) {
        ...
        that.$ = $(template.format(options)).toggleClass("fade", options.fade).toggleClass("scale", options.scale)
            .attr("id", "messager-" + that.id);
...
(function($, Math, window, undefined) {
    ...
    function isColor(hex) {
        return typeof(hex) === STR_STRING &&
        (hex.toLowerCase() === "transparent" ||
            namedColors[hex.toLowerCase()] ||
            hexReg.test($.trim(hex.toLowerCase())));
    }
...
(function($) {
        ...
        if ($.isPlainObject(actions)) {
            ...
            $.each(actions,
                function(name, action) {
                    if (action) {
                        _actions[name] = $.extend({ type: name },
    ...
    function createActionEle(action, name, template) {
        ...
        return $(template || action.template).addClass("tree-action")
            .attr($.extend({ 'data-type': name, title: action.title || "" }, action.attr)).data("action", action);
    }
            ...
            $.each(items,
                function(idx, item) {
                    ...
                    var $wrapper = options.itemWrapper
                        ? $(options.itemWrapper === true ? '<div class="tree-item-wrapper"/>' : options.itemWrapper)
                        .appendTo($li)
                        : $li;
                    if (item.html) {
            ...
            if (actions.add && actions.add.templateInList !== false) {
                ...
                if (!$actionItem.length) {
                    $('<li class="tree-action-item"/>')
                        .append(createActionEle(actions.add, "add", actions.add.templateInList)).appendTo($list);
                } else {
                ...
                $list.sortable($.extend({
                        ...
                        finish: function(e) {
                            that.callEvent("action",
                                { action: actions.sort, $list: $list, target: e.target, item: data });
                        }
                    },
                    actions.sort.options,
                    $.isPlainObject(this.options.sortable) ? this.options.sortable : null));
            }
        }
        ...
        if (actions) {
            ...
            if (!$actions.length) {
                $actions = $('<div class="tree-actions"/>')
                    .appendTo(this.options.itemWrapper ? $item.find(".tree-item-wrapper") : $item);
                $.each(actions,
    ...
    Tree.prototype.toggle = function($li) {
        var collapse = ($li && $li.hasClass("open")) ||
            $li === false ||
